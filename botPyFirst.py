...
from telegram.ext import Application, CommandHandler, MessageHandler, filters, CallbackContext, Updater, CallbackQueryHandler
import random
...
async def start(update: Update, context: CallbackContext):
    await update.message.reply_text(
        ...
        "1. Помогать вам задавать друг другу интересные вопросы.\n"
        "2. Позволить вам выбрать удобное время для вопросов.\n"
        "3. Делиться вашими ответами друг с другом.\n"
        "Введите /key, чтобы создать общий ключ для вашей пары.\n"
...
async def help(update: Update, context: CallbackContext):
    await update.message.reply_text(
        ...
        "/set_time - Установить время для вопросов (13:00, 17:00, 21:00)\n"
...
async def create_key(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(f"Ваш общий ключ: {key}. Передайте его вашему партнеру.")

# Ввод ключа для подключения
async def join_key(update: Update, context: CallbackContext):
    try:
        partner_key = int(context.args[0])
        for user_id, data in user_data.items():
            if data.get("key") == partner_key:
                ...
                context.bot.send_message(chat_id=user_id, text="Ваш партнер подключился!")
                return
...
# Обработка нажатия кнопок выбора времени
async def set_time_callback(update: Update, context: CallbackContext):
    query = update.callback_query
    await query.answer()
    time_choice = query.data
    user_data[query.message.chat_id]["time"] = time_choice
    await query.edit_message_text(f"Вы выбрали время: {time_choice}")

# Функция для обработки команды, если команда не существует
async def unknown(update: Update, context: CallbackContext):
    await update.message.reply_text("Извините, я не понимаю эту команду. Используйте /help, чтобы увидеть все доступные команды.")
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(set_time_callback))
